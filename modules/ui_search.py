        ...
        with st.spinner("☁️ Φόρτωση Βιβλιοθήκης..."):
            try:
                ...
                if not st.session_state.library_cache:
                    st.info(f"ℹ️ {get_text('lic_admin_no_licenses', lang)} Πατήστε 'Sync' για αρχικοποίηση.") # Re-using key
            except Exception as e:
                logger.error(f"UI Search: Error loading library index: {e}", exc_info=True)
                st.error(f"❌ Error loading library index: {e}")
